...
<form action="{{ route('posts.update', $post->id) }}" method="POST" class="m-auto w-75 mt-5">
    ...
    <div class="mb-3">
        ...
        <select class="form-select form-select-lg" name="post_creator" id="" disabled>
            @foreach($users as $user)
              <option value="{{ $user->id }}" <?php if($user->id == $post->user_id ) echo 'selected'; ?> > {{ $user->name }} </option>
            @endforeach
        </select>
    </div>
    ...
    <div class="mb-3">
      ...
      <input type="datetime-local" class="form-control"  name="created_at" value="{{ $post->created_at }}" id="" disabled>
    </div>
